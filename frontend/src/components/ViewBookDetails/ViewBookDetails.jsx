...
const ViewBookDetails = () => {
        ...
        const fetch = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:3000/api/v1/get-book-byid/${id}`
                );
    ...
    const handleFavourite = async () => {
        try {
            const response = await axios.put(
                "http://localhost:3000/api/v1/add-book-favourite",
                {},
                { headers }
            );
            setResponseMessage(response.data.message); // Set the popup message
            setTimeout(() => setResponseMessage(''), 3000); // Auto-hide after 3 seconds
        } catch (error) {
            console.error("Error adding book to favourites", error);
            setResponseMessage("Failed to add book to favourites."); // Show error message
            setTimeout(() => setResponseMessage(''), 3000); // Auto-hide after 3 seconds
        }
    };
    ...
    const handleCart = async () => {
        try {
            const response = await axios.put(
                "http://localhost:3000/api/v1//add-book-cart",
                {},
                { headers }
            );
            setCartResponseMessage(response.data.message);
            setTimeout(() => setCartResponseMessage(''), 3000);
        } catch (error) {
    ...
    const deletebook = async () => {
        const response = await axios.delete(
            "http://localhost:3000/api/v1/delete-book", { headers }
        );
