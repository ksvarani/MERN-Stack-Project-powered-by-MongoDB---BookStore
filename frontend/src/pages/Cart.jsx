...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        "http://localhost:2004/api/v1/get-book-cart", { headers }
      );
  ...
  const deleteItem = async (bookid) => {
    const response = await axios.put(
      `http://localhost:2004/api/v1/remove-book-cart/${bookid}`, {}, { headers }
    );
  ...
  const PlaceOrder = async () => {
    try {
      const response = await axios.post(
        "http://localhost:2004/api/v1/place-order", {order: Cart}, { headers }
      );
      //alert(response.data.message);
